...
async function handler(m, { conn: _envio, usedPrefix }) {
  ...
  const replyMessage = message.length === 0 ? '*⚠️ NO HAY SUB BOTS DE NIGROMANTEBOT-MD DISPONIBLE POR EL MOMENTO . 🔰VERIFIQUE MÁS TARDE.*' : message;
  const totalUsers = users.length;
  const responseMessage = `${replyMessage.trim()}`.trim();
await m.reply(`*🚀 AQUÍ TIENE LA LISTA DE LOS SUBBOTS ACTIVÓS EN ESTOS MOMENTOS*\n\nSUB BOTS DE NIGROMANTEBOT-MD CONECTADOS : ${totalUsers || '0'}`)
