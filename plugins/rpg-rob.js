...
let handler = async (m, {conn, text, usedPrefix, command, groupMetadata}) => {
  ...
  if (new Date() - global.db.data.users[m.sender].lastrob < 1800000) throw `*⏰ ESPERA ${msToTime(time - new Date())} PARA VOLVER A ROBAR NECROMANCIA*`;
  if (!text) return m.reply(`*➳ ETIQUETA AL USUARIO QUE QUIERES ROBARLE SUS NECROMANCIAS*\n\n*EJEMPLO:* ${usedPrefix + command} @tɑg>.`);
  try { 
...
ʀᴏʙᴀᴅᴏ ᴘᴏʀ: @${m.sender.split("@")[0]}`;
    ...
    await m.reply(`*🚓🚓🚓No le pudiste robar por que a este usuario los protege el dios nigromante 👮(AFK)*`);
  }
};
